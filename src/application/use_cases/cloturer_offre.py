"""Use case pour clôturer une offre."""
...
class CloturerOffreUseCase:
    """Use case pour clôturer une offre (passer de OUVERTE à CLOTUREE)."""
    ...
    async def executer(self, offre_id: str) -> OffreDTO:
        """Clôture une offre et retourne ses informations complètes."""
        offre = await self._offre_repository.obtenir_par_id(UUID(offre_id))
        if not offre:
            raise OffreIntrouvableError(offre_id)

        # offre.cloturer() lève ValueError si la transition est invalide
        # (seules les offres ouvertes peuvent être clôturées).
        offre.cloturer()

        offre_sauvegarde = await self._offre_repository.sauvegarder(offre)

        # Mettre à jour l'index (statut CLOTUREE + date_cloture) — best-effort
